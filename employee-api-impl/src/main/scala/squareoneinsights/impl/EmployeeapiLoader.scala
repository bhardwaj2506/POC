...
import squareoneinsights.api.{EmployeeService, EmployeeapiService}
import com.lightbend.lagom.scaladsl.broker.kafka.LagomKafkaComponents
import com.lightbend.lagom.scaladsl.playjson.JsonSerializerRegistry
import com.softwaremill.macwire._
import squareoneinsights.impl.employee.EmployeeServiceImpl
...
abstract class EmployeeapiApplication(context: LagomApplicationContext)
  ...
  clusterSharding.init(
    Entity(EmployeeapiState.typeKey)(
      entityContext => EmployeeapiBehavior.create(entityContext)
    )
  )
