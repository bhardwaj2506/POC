...
import squareoneinsights.api.AddEmployeeRequest

import scala.collection.immutable.Seq

/**
  * This provides an event sourced behavior. It has a state, [[EmployeeapiState]], which
  ...
  */
object EmployeeapiBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[EmployeeapiCommand]): Behavior[EmployeeapiCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, EmployeeapiEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[EmployeeapiCommand, EmployeeapiEvent, EmployeeapiState](
        persistenceId = persistenceId,
        emptyState = EmployeeapiState.initial,
...
/**
  ...
  */
case class EmployeeapiState(message: String, timestamp: String) {
  def applyCommand(cmd: EmployeeapiCommand): ReplyEffect[EmployeeapiEvent, EmployeeapiState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: EmployeeapiEvent): EmployeeapiState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[EmployeeapiEvent, EmployeeapiState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[EmployeeapiEvent, EmployeeapiState] =
    Effect
...
object EmployeeapiState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: EmployeeapiState = EmployeeapiState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[EmployeeapiCommand]("EmployeeapiAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[EmployeeapiState] = Json.format
}

/**
  * This interface defines all the events that the EmployeeapiAggregate supports.
  */
sealed trait EmployeeapiEvent extends AggregateEvent[EmployeeapiEvent] {
  def aggregateTag: AggregateEventTag[EmployeeapiEvent] = EmployeeapiEvent.Tag
}

object EmployeeapiEvent {
  val Tag: AggregateEventTag[EmployeeapiEvent] = AggregateEventTag[EmployeeapiEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends EmployeeapiEvent
...
/**
  ...
  */
trait EmployeeapiCommandSerializable

/**
  * This interface defines all the commands that the EmployeeapiAggregate supports.
  */
sealed trait EmployeeapiCommand
    extends EmployeeapiCommandSerializable
...
object EmployeeapiSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    ...
    JsonSerializer[EmployeeapiState],
